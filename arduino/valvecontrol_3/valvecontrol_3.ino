...
unsigned long interval_3=4000; // sau 4s bat van 2
...
void signal_func(){
  ...
  if (!q.isEmpty())        // Only if q is not empty
  {
    ...
    if (label == '0') {                 // neu toi la "bad"                           
      if ((unsigned long)(currentMillis - prevMillis0_ON) >= interval_2) {     // valve0 ON after 5s
      ...
      if ((unsigned long)(currentMillis - prevMillis2_ON) >= interval_3) {       // valve2 ON after 4s
